...
def main():
    ...
    srna_csv = pd.read_csv('/home/tara/paper_revisions/srna/srna_calc.csv')  # THIS FILE IS OBTAINED FROM SUPPLEMENTAL MATERIAL https://doi.org/10.1093/nar/gkv1477 , with an appended column "all possible base pairs", calculated using TN equation from paper



    # Path where dinoknot outputs are located
    srna_input_path = '/home/tara/paper_revisions/srna/dot_bracket_dinoknot_output/400_cases_per_interaction_structures'




    # Get the list of files - one file per interaction
    srna_file_list = glob.glob(f'{srna_input_path}/*srna_output.txt')
 

    

    # do process for srna then repeat for snorna
    for srna_interaction in srna_file_list: 

        with open(srna_interaction, 'r') as srna_input_file:

            pair_id = int(re.search(r'(\d+)_srna_output\.txt', srna_interaction).group(1)) if re.search(r'(\d+)_srna_output\.txt', srna_interaction) else None

            #print("pair_id:", pair_id)
            srna_lines = srna_input_file.readlines()
            # Change: read first line instead of list comp

            try:
                sequence = [line.split()[1].strip() for line in srna_lines if line.startswith("Seq")][0]
            except:
                continue
            srna_result_lines = [line for line in srna_lines if line.startswith("Result")]
            tokenized_result_lines = [line.split() for line in srna_result_lines]
            ...
            for i, (structure, free_energy) in enumerate(structure_energy_pairs, start=0):
                ...
                srna_full_bpseq_file_path = '/home/tara/paper_revisions/srna/output_bpseq/400_cases_per_interaction_structures/full_bpseq/interaction_' + str(pair_id) + '_result_' + str(i) + '_dinoknot_output_full_bpseq.csv'
                
                with open(srna_full_bpseq_file_path, 'w') as output_file:
                # Iterate through the list and write each item to the file
                    for item in bpseq:
                        output_file.write(item)

                filtered_dinoknot_bpseq = filter_bpseq_to_srna_interaction_site(srna_csv, bpseq, i, pair_id)

                ## import filtered reference


                # Specify the path to the file
                filtered_reference_bpseq_path = "/home/tara/paper_revisions/srna/reference_bpseq/filtered_bpseq/" + str(pair_id) + "_filtered_srna_ref_output.bpseq" 
                ...
                TP, FP, FN, TN, TPR, PPV, MCC = calculate_metrics(filtered_reference_bpseq, filtered_dinoknot_bpseq, pair_id, srna_csv)
    ...
    final_output_df.to_csv("/home/tara/paper_revisions/srna/calculations/400_cases_per_interaction_structures/srna_performance_calculations.csv", index = False)        
