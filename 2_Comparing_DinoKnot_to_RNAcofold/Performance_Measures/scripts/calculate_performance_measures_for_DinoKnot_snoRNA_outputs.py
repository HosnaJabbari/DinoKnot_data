...
def main():
    ...
    final_output_file_headers = ['interaction', 'result', 'free_energy', 'TP', 'FP', 'FN', 'TN', 'TPR', 'PPV', 'MCC']
    final_combined_output_list = []

    # Read in master ssnorna csv files with interaction, positions, expected structure etc.
    snorna_csv = pd.read_csv('/home/tara/paper_revisions/snorna/snorna_calc.csv') # THIS FILE IS OBTAINED FROM SUPPLEMENTAL MATERIAL https://doi.org/10.1093/nar/gkv1477 , with an appended column "all possible base pairs", calculated using TN equation from paper


    # Path where dinoknot outputs are located
  
    snorna_input_path = '/home/tara/paper_revisions/snorna/dot_bracket_dinoknot_output/400_cases_per_interaction_structures'
    ...
    # do process for srna then repeat for snorna
    for snorna_interaction in snorna_file_list: 

        with open(snorna_interaction, 'r') as snorna_input_file:
          
            pair_id = int(re.search(r'(\d+)_snorna_output\.txt', snorna_interaction).group(1)) if re.search(r'(\d+)_snorna_output\.txt', snorna_interaction) else None
            ...
            try:
                sequence = [line.split()[1].strip() for line in snorna_lines if line.startswith("Seq")][0]
            except:
                continue
            snorna_result_lines = [line for line in snorna_lines if line.startswith("Result")]
            tokenized_result_lines = [line.split() for line in snorna_result_lines]
            structure_energy_pairs = [(tokens[1], tokens[2]) for tokens in tokenized_result_lines]
            
            #assert in for loop for all structures ***
            #assert len(sequence) == len(structure_energy_pairs[0][0]), "The length of predicted structure doesn't match the length of input sequence"

            ## use i to keep count of which structure we loop through for file output
            for i, (structure, free_energy) in enumerate(structure_energy_pairs, start=0):
                ...
                snorna_full_bpseq_file_path = '/home/tara/paper_revisions/snorna/output_bpseq/400_cases_per_interaction_structures/full_bpseq/interaction_' + str(pair_id) + '_result_' + str(i) + '_dinoknot_output_full_bpseq.csv'
                ...
                filtered_dinoknot_bpseq = filter_bpseq_to_snorna_interaction_site(snorna_csv, bpseq, i, pair_id)

                ## import filtered reference


                # Specify the path to the file
                filtered_reference_bpseq_path = "/home/tara/paper_revisions/snorna/reference_bpseq/filtered_bpseq/" + str(pair_id) + "_filtered_snorna_ref_output.bpseq" 
                ...
                #print(filtered_reference_bpseq[0:10])

                TP, FP, FN, TN, TPR, PPV, MCC = calculate_metrics(filtered_reference_bpseq, filtered_dinoknot_bpseq, pair_id, snorna_csv)

                final_result_row = [pair_id, i, free_energy.strip("(").strip(")"), TP, FP, FN, TN, TPR, PPV, MCC]
                #print(final_result_row)
    ...
    final_output_df.to_csv("/home/tara/paper_revisions/snorna/calculations/400_cases_per_interaction_structures/snorna_performance_calculations.csv", index = False)        




def filter_bpseq_to_srna_interaction_site(df, bpseq, i, pair_id):

    outpath = '/home/tara/paper_revisions/srna/output_bpseq/400_cases_per_interaction_structures/filtered_bpseq'
    ...
    if df['pair_id'].isin([pair_id]).any():
                ...
                for row in bpseq:
                    ...
                    if (index >= srna_site_start and index <= srna_site_end) or \
                               (index >= (mrna_site_start + srna_length + 5) and index <= (mrna_site_end + srna_length + 5)): # +5 handles linker
...
def filter_bpseq_to_snorna_interaction_site(df, bpseq, i, pair_id):

    outpath = '/home/tara/paper_revisions/snorna/output_bpseq/400_cases_per_interaction_structures/filtered_bpseq'
    ...
    if df['pair_id'].isin([pair_id]).any():
                ...
                for row in bpseq:
                    index = int(row.split()[0])
                            # Check if the row falls within the snoRNA or mRNA range
                ...
                # Write the filtered rows to the output directory with the modified filename
                output_filename = f"interaction_{pair_id}_result_{i}_filtered_snorna_dinoknot_output.bpseq"
                #with open(os.path.join(outpath, output_filename), 'w') as outfile:
                 #   outfile.writelines(filtered_rows)
