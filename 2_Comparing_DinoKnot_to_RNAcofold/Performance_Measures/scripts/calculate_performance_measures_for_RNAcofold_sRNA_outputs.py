...
def main():
    ...
    srna_csv = pd.read_csv('/home/tara/paper_revisions/RNAcofold/srna/srna_calc.csv') # THIS FILE IS OBTAINED FROM SUPPLEMENTAL MATERIAL https://doi.org/10.1093/nar/gkv1477 , with an appended column "all possible base pairs", calculated using TN equation from paper



    # Path where dinoknot outputs are located
    srna_input_path = '/home/tara/paper_revisions/RNAcofold/srna/dot_bracket_RNAcofold_output/single_mfe_structures'
    ...
    for srna_interaction in srna_file_list: 
        ...
        with open(srna_interaction, 'r') as srna_input_file:
            #print(srna_interaction)

            pair_id = int(re.search(r'(\d+)_RNAcofold_sRNA_output\.txt', srna_interaction).group(1)) if re.search(r'(\d+)_RNAcofold_sRNA_output\.txt', srna_interaction) else None
            ...
            sequence = srna_lines[1].strip()
            sequence = sequence.replace("&", "")
            #print(sequence)

            structure_line = srna_lines[2]
            ...
            srna_full_bpseq_file_path = '/home/tara/paper_revisions/RNAcofold/srna/output_bpseq/single_mfe_structures/full_bpseq/interaction_' + str(pair_id) + '_RNAcofold_output_full_bpseq.csv'
            ...
            filtered_dinoknot_bpseq = filter_bpseq_to_srna_interaction_site(srna_csv, bpseq, pair_id)

            ## import filtered reference


            # Specify the path to the file
            filtered_reference_bpseq_path = "/home/tara/paper_revisions/RNAcofold/srna/reference_bpseq/filtered_bpseq/" + str(pair_id) + "_filtered_srna_ref_output.bpseq" 
    ...
    final_output_df.to_csv("/home/tara/paper_revisions/RNAcofold/srna/calculations/single_mfe_structures/srna_performance_calculations.csv", index = False)        
    ...
    if df['pair_id'].isin([pair_id]).any():
                ...
                for row in bpseq:
                    ...
                    if (index >= srna_site_start and index <= srna_site_end) or \
                               (index >= (mrna_site_start + srna_length ) and index <= (mrna_site_end + srna_length )):  # +5 handles linker
                                filtered_rows.append(row)
                
                # Write the filtered rows to the output directory with the modified filename
                output_filename = f"interaction_{pair_id}_result_filtered_srna_RNAcofold_output.bpseq"
